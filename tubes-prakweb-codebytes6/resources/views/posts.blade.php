...
<div class="card mb-3">
   ...
   <div style="max-height: 350px; margin-left:auto; margin-right:auto; display:block; ">
      <img src="{{ asset('storage/' . $posts[0]->image) }}" alt="{{ $posts[0]->category->name }}" class="img-fluid" style="width: 400px; height: 400px;">
   </div>
   @else
   ...
   <div class="card-body text-center">
      ...
      <p>
         <small class="text-muted">
            by. <a href="/post?author={{$posts[0]->author->username }}" class="text-decoration-none"> {{$posts[0]->author->name }}</a> in <a href="/post?category={{$posts[0]->category->slug }}" class="text-decoration-none">{{$posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }}
         </small>
      </p>
